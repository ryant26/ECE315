...
void Lcd::Init(unsigned char screen)
{
	...
	if (sem_inited == TRUE){
		...
		if (err == OS_NO_ERR)
		{
			...
			{
				case LCD_UPPER_SCR :
					send_cmd(LCD_UPPER_SCR, CMD_FUNCTION | FUNCTION_8BIT | FUNCTION_2LINE | FUNCTION_5x8);
					send_cmd(LCD_UPPER_SCR, CMD_CLEAR);
					send_cmd(LCD_UPPER_SCR, CMD_DISPLAY | DISPLAY_ON | DISPLAY_NOCURSOR | DISPLAY_BLINK);
					send_cmd(LCD_UPPER_SCR, CMD_ENTRY_MODE | ENTRY_CURSOR_INC | ENTRY_NOSHIFT);
					send_cmd(LCD_UPPER_SCR, CMD_SHIFT | SHIFT_CURSOR | SHIFT_RIGHT);
